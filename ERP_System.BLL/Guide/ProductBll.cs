...
namespace ERP_System.BLL.Guide
{
    public class ProductBll
    {
        ...
        {
            string fileName = "D:\\ERP DB Scripts\\Products.xlsx";
            var basePath = "D:\\ERP DB Scripts\\productQuieries.txt";
            using (var excelWorkbook = new XLWorkbook(fileName))
            {
                var nonEmptyDataRows = excelWorkbook.Worksheet(1).RowsUsed();
                var query = "";
                var StockProductsQuery = "";
                ...
                using (StreamWriter sw = new StreamWriter(basePath))
                {
                    foreach (var dataRow in nonEmptyDataRows)
                    {
                        ...
                        {
                            var ProductName = dataRow.Cell(7).Value.ToString().Trim();
                            var UnitName = dataRow.Cell(2).Value.ToString().Trim();
                            var ProductBarCode = dataRow.Cell(14).Value.ToString().Trim();
                            var ItemGroupName = dataRow.Cell(1).Value.ToString().Trim();
                            var Description = dataRow.Cell(8).Value.ToString().Trim();

                            if (!string.IsNullOrEmpty(ProductName))
                            {
                                query = createProductQuery(ProductName, ProductBarCode, ItemGroupName, Description , UnitName);
                                StockProductsQuery = createStockProductQuery(ProductName);
                                ProductUnitQuery = createProductUnitsQuery(ProductName, ProductBarCode, UnitName);
                                ...
                                sw.WriteLine(query);
                                sw.WriteLine();
                                sw.WriteLine(StockProductsQuery);
                                sw.WriteLine();
                                sw.WriteLine(ProductUnitQuery);
                                sw.WriteLine();
        ...
        private string createProductQuery(string productName, string Barcode, string ItemGroupName, string Description,string UnitName)
        {
            string query = @$"
                    INSERT INTO [Guide].[Products]
                               ([ID]
                               ,[Name]
                               ,[BarCodeText]
                               ,[BarCodePath]
                         ...
                         VALUES
                               ...
                               ,N'{Description}'
                               ,null
                               ,(select top 1 p.ID from Guide.Units p where p.Name=N'{UnitName}')
                               ,null
                               ,(select top 1 ID from Guide.ItemGrpoups where Name =N'{ItemGroupName}')
                               ,getDate()
                               ,'80968C16-15D8-4533-B771-5285299EDCB6'
                               ,null
                               ,null
                               ,0
                               ,1
        ...
        private string createStockProductQuery(string productName)
        {
            string query = @$"
                    INSERT INTO [Guide].[StockProducts]
                               ([ID]
                               ,[StockId]
                               ,[ProductId]
                               ,[CreatedDate]
                         ...
                         VALUES
                               (newid()
                               ,N'2150E6F3-8EBE-4DAB-121C-08DBB3B1CF7F'
                               ,(select top 1 p.ID from Guide.Products p where p.Name = N'{productName}')
                               ,getdate()
                               ,N'80968C16-15D8-4533-B771-5285299EDCB6'
                               ,null
                               ,null
                               ,0
                               ,1
        ...
        private string createProductUnitsQuery(string productName, string ProductBarcode, string unitName)
        {
            string query = @$"
                    INSERT INTO [Guide].[ProductUnits]
                       ([ID]
                       ,[ConversionFactor]
                       ,[PurchasingPrice]
                       ,[SellingPrice]
